...
class SecureFHSSAlgorithm:
    ...
    Bu algoritma hem gönderici hem alıcı tarafında aynı şekilde çalışır yani gönderici ve alıcı birbirine bağlı değil aynı algoritmaya bağlı olarak çalışır.
    
    Gürültü toleransı için Türkiye'den LEO uydularına (400-800 km) sinyal gönderiminde gürültü oranı %0.99 . Genlik toleransı (2.7V-3.3V), zaman toleransı 
    (1.9-2.1 ms) ve 40 MHz frekans toleransı, gürültüyü tolere eder.
    
    Güvenlik için 40 MHz tolerans, 2000-4000 MHz bandında 2000 kanaldan sadece %2'lik bir hedef alanı kapsar. 2 ms hızlı atlama ve şifreli, değişken 
    k₁×k₂ sırası düşmanın sinyali taklit etmesinide engeller.
    """
    
    def __init__(self, sync_key=12345):
        """
        Algoritma değerlerini başlatması için lazım bu.
        ...
        # sabit algoritma değerleri
        self.f0 = 2000  # MHz - S Bandı başlangıcı
        self.t_hop = 2  # ms - Sabit atlama süresi
        self.band_min = 2000  # MHz - S Bandı alt limit
        self.band_max = 4000  # MHz - S Bandı üst limiti
        self.nominal_amplitude = 3.0  # V - Nominal genliği
        self.amplitude_tolerance = 0.3  # V - Sabit genlik toleransı
    ...
    def calculate_frequency(self, amplitude, time_ms, energy):
        ...
        while attempts < max_attempts:
            ...
            # Eğer bant dışındaysa listedeki bir sonraki k1×k2 değerini denemesi için
            self.current_k_index = (self.current_k_index + 1) % len(self.k1_k2_list)
            attempts += 1
        
        # Eğer hiçbiri bir işe yaramazsa (çooook nadir bir durum)
    ...
    def simulate_sender_operation(self, hop_count=10):
        ...
        for i in range(hop_count):
            ...
            # simülasyon için kısacık bir bekleme
            time.sleep(0.001)
